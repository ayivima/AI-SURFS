...
class Matrix():
    ...
    def __getitem__(self, key):
        return self.rows[key].components
    ...
    def __ne__(self, operand):
        return self.rows!=operand.rows

    def __neg__(self):
        return Matrix(
            [vector.reversed().components for vector in self.rows]
        )
        
    def __pow__(self, power):
        return Matrix(
            [pow(vector, power).components for vector in self.rows]
        )
    ...
    def __smoothen__(self):
        ...
        for index in range(self.row_size):
            self.rows[index].pad(self.column_size)
    ...
    def matmul(self, other):
        if isinstance(other, Matrix):
            if self.shape[1]==other.shape[0]:
                ...
                result = []
                other_components = [vector.components for vector in other.rows]
                
                other_prep = Matrix([
                    list(tuple) for tuple in zip(*other_components)
                ])
    ...
    def flattened(self):
        flat_matrix = self.rows[0]

        for index in range(1, self.row_size):
            flat_matrix = flat_matrix.concat(self.rows[index])

        return Matrix([flat_matrix.components])
